document.addEventListener('DOMContentLoaded', () => {
  const containers = document.querySelectorAll('.logo-interactive-wrapper');
  ...
  containers.forEach(container => {
    ...
    if (orbit1 && orbit2) {
      gsap.to([orbit1, orbit2], {
        attr: { rx: 20, ry: 30 }, // Slightly smaller breathing for the new size
        duration: 2,
        repeat: -1,
        yoyo: true,
        ease: "sine.inOut",
    ...
    container.addEventListener('mouseleave', () => {
      ...
      if (outerRing) gsap.set(outerRing, { opacity: 1 });
      ...
      if (orbit1 && orbit2) {
        gsap.to([orbit1, orbit2], {
          ...
          attr: { rx: 22, ry: 32 }, // Return to original size
          duration: 1,
          ease: "power2.out"
        });
      }
